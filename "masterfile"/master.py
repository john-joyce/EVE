...
class Morbidostat:
    ...
    def dyn_pump_waste(self):       #NEW
        global self.vial_drug_mass
        global self.drug_target_mL
          if self.target_concentration > ((self.vial_drug_mass)/self.culture_vol) or ((self.vial_drug_mass)/self.culture_vol) == 0:
            self.drug_target_mL = ((self.culture_vol)/100) * (-((50 * (-((self.vial_drug_mass)/self.culture_vol) * self.drug_conc + math.sqrt(((((self.vial_drug_mass)/self.culture_vol)** 2) * (self.drug_conc ** 2)) - (4*((self.vial_drug_mass)/self.culture_vol)*(self.fluct_rate * self.target_concentration)) ) )))/(((self.vial_drug_mass)/self.culture_vol) * self.drug_conc))
            self.P_dyn_waste_times = (self.drug_target_mL / self.drug_pump_flo_rate)
          else:
            self.media_target_mL = -(((self.culture_vol * (self.fluct_rate * self.target_concentration)) - (self.culture_vol * ((self.vial_drug_mass)/self.culture_vol)))/((self.vial_drug_mass)/self.culture_vol))
